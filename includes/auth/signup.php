...
if(isset($_POST['register'])){
    ...
    $referrer_hashed = $_GET["ref"];
    $ref_code_check = $_POST['ref_code'];
    $referrer_select = "SELECT member_id FROM ACCOUNTS";
    $referrer_query = mysqli_query($conn, $referrer_select);
    
    // if($member_id == $member_id) {
    // if(password_verify($password, $referrer_hashed)){
        ...
        if ($referral_count == 1) {
            ...
            while($referral_info = mysqli_fetch_assoc($resultReferral)) {
                ...
                if($create_user_count == 0) {
                    if(($pass == $confirm_pass)) {
                        ...
                        if($member_id == $member_id) {
                        // if(password_verify($member_id, $referrer_hashed)){
                            $id_hash = password_hash($member_id, PASSWORD_DEFAULT);
                            echo "<script> console.log('$id_hash') </script>";

                            //Change this format when deployed
                            $referralLink = "http://localhost/ArvieCSP/signup.php?ref=$id_hash";
            ...
            echo "<script> alert('This code does not exist or already been used2.')</script>";
        }
    // }else {
    //     echo "<script> alert('This code does not exist or already been used1.')</script>";
    // }
}
